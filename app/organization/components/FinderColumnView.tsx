...
function SortableOrgItem({
  ...
  // 日本語変換中かどうかを追跡するref（同期的にチェックするため）
  ...
  // 編集用のキーハンドラー（日本語変換中はEnterキーで保存しない）
  const handleEditKeyDown = (e: React.KeyboardEvent) => {
    // すべてのキーイベントを親要素に伝播させない（ドラッグ操作を防ぐため）
    e.stopPropagation();
    
    // 日本語変換中はEnterキーで保存しない
    if (isComposingRef.current && e.key === 'Enter') {
      return;
    }
    
    // EnterキーとEscapeキーのみ処理
    if (e.key === 'Enter') {
      e.preventDefault();
      if (editingOrgId === org.id && editingOrgName.trim()) {
        onEditSave(org.id!, editingOrgName.trim());
      }
    ...
    // その他のキー（スペースキーなど）は通常通り処理されるが、親要素には伝播しない
  };
