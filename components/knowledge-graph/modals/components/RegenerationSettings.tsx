                ...
                <div style={{ marginLeft: '8px' }}>
                  {(() => {
                    ...
                    if (selectedType === 'all' || selectedType === 'entities') {
                      const total = missingCounts.totalEntities || 0;
                      counts.push(`エンティティ: ${missingCounts.entities}件 / ${total}件`);
                    }
                    if (selectedType === 'all' || selectedType === 'relations') {
                      const total = missingCounts.totalRelations || 0;
                      counts.push(`リレーション: ${missingCounts.relations}件 / ${total}件`);
                    }
                    if (selectedType === 'all' || selectedType === 'topics') {
                      const total = missingCounts.totalTopics || 0;
                      counts.push(`トピック: ${missingCounts.topics}件 / ${total}件`);
                    }
                      ...
                      <>
                        ...
                        {selectedType === 'all' && (
                          <div style={{ marginTop: '4px', fontWeight: 600, borderTop: '1px solid #93C5FD', paddingTop: '4px' }}>
                            合計: {missingCounts.total}件 / {(missingCounts.totalEntities || 0) + (missingCounts.totalRelations || 0) + (missingCounts.totalTopics || 0)}件
                          </div>
                        )}
                      </>
                    );
