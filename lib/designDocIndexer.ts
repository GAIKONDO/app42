...
【ユーザー管理系】
- users: ユーザー情報（id, email, passwordHash, approved, approvedBy, approvedAt, role, createdAt, updatedAt）
- approvalRequests: 承認リクエスト（id, userId, email, status, requestedAt）

【組織管理系】
- organizations: 組織情報（id, parentId, name, title, description, level, levelName, position, type, createdAt, updatedAt）
- organizationMembers: 組織メンバー情報（id, organizationId, name, position, nameRomaji, department, extension, companyPhone, mobilePhone, email, itochuEmail, teams, employeeType, roleName, indicator, location, floorDoorNo, previousName, createdAt, updatedAt）
- organizationContents: 組織コンテンツ（id, organizationId, introduction, focusAreas, meetingNotes, createdAt, updatedAt）

【事業会社系】
- companyContents: 事業会社コンテンツ（id, companyId, introduction, focusBusinesses, capitalStructure, capitalStructureDiagram, createdAt, updatedAt）
- 注意: companiesテーブルとorganizationCompanyDisplayテーブルは削除され、organizationsテーブルに統合されました

【議事録・施策系】
- meetingNotes: 議事録（id, organizationId, companyId, title, description, content, chromaSynced, chromaSyncError, lastChromaSyncAttempt, createdAt, updatedAt）
- focusInitiatives: 注力施策（id, organizationId, companyId, title, description, content, themeIds, topicIds, createdAt, updatedAt）
- themes: テーマ（id, title, description, initiativeIds, createdAt, updatedAt）
- themeHierarchyConfigs: テーマ階層設定（id, maxLevels, levels, createdAt, updatedAt）

【ナレッジグラフ系】
- entities: エンティティ（人、組織、概念など）（id, name, type, aliases, metadata, organizationId, companyId, searchableText, displayName, chromaSynced, chromaSyncError, lastChromaSyncAttempt, lastSearchDate, searchCount, createdAt, updatedAt）
- relations: エンティティ間の関係（id, topicId, sourceEntityId, targetEntityId, relationType, description, confidence, metadata, organizationId, companyId, searchableText, chromaSynced, chromaSyncError, lastChromaSyncAttempt, lastSearchDate, searchCount, createdAt, updatedAt）
- topics: トピックのメタデータ（id, topicId, meetingNoteId, organizationId, companyId, title, description, content, semanticCategory, keywords, tags, searchableText, contentSummary, chromaSynced, chromaSyncError, lastChromaSyncAttempt, lastSearchDate, searchCount, imagePaths, parentTopicId, createdAt, updatedAt）
- topicFiles: トピックファイル（id, topicId, parentTopicId, filePath, fileName, mimeType, description, detailedDescription, fileSize, organizationId, companyId, meetingNoteId, createdAt, updatedAt）

【システム設計ドキュメント系】
- designDocSections: システム設計ドキュメントセクション（id, title, description, content, tags, order_index, pageUrl, hierarchy, relatedSections, semanticCategory, keywords, summary, createdAt, updatedAt）
- designDocSectionRelations: セクション間の関係（id, fromSectionId, toSectionId, relationType, description, createdAt, updatedAt）

【Agentシステム系】
- agents: Agent定義（id, name, description, role, capabilities, tools, modelType, selectedModel, systemPrompt, config, createdAt, updatedAt）
- tasks: タスク（id, agentId, type, title, description, status, priority, config, result, createdAt, updatedAt）
- taskExecutions: タスク実行履歴（id, taskId, agentId, status, result, error, startedAt, completedAt, createdAt）
- taskChains: タスクチェーン（id, name, description, startNodeId, nodes, createdAt, updatedAt）
- a2aMessages: A2Aメッセージ履歴（id, fromAgent, toAgent, type, taskId, payload, timestamp, responseTo, requiresResponse, createdAt）
- agent_prompt_versions: Agentプロンプトバージョン履歴（id, agentId, version, systemPrompt, createdAt, updatedAt）
- mcp_tools: MCPツール（id, name, description, arguments, returns, implementationType, enabled, createdAt, updatedAt）

【その他】
- aiSettings: AI設定（id, provider, apiKey, baseUrl, defaultModel, createdAt, updatedAt）
- backupHistory: バックアップ履歴（id, backupPath, backupSize, createdAt）
...
- organizations ||--o{ focusInitiatives: belongs_to
- meetingNotes ||--o{ topics: contains
- meetingNotes ||--o{ topicFiles: has
- topics ||--o{ relations: has
- topics ||--o{ topicFiles: has
- topics }o--o{ topics: parent_child
- topics }o--|| entities: referenced_by_metadata
- entities ||--o{ relations: source
- entities ||--o{ relations: target
- agents ||--o{ tasks: has
- agents ||--o{ taskExecutions: has
- agents ||--o{ agent_prompt_versions: has
- tasks ||--o{ taskExecutions: has
...
- topics.companyId → organizations.id（type='company'の場合）
- entities.organizationId → organizations.id
- entities.companyId → organizations.id（type='company'の場合）
- topicFiles.topicId → topics.id
- topicFiles.meetingNoteId → meetingNotes.id
- focusInitiatives.organizationId → organizations.id
- focusInitiatives.companyId → organizations.id（type='company'の場合）
- meetingNotes.organizationId → organizations.id
- meetingNotes.companyId → organizations.id（type='company'の場合）

RAG検索最適化カラム:
- entities, relations, topicsテーブルには以下のRAG検索最適化カラムが追加されています:
  - searchableText: 検索可能なテキスト（自動生成）
  - displayName: 表示名（entitiesのみ、自動生成）
  - contentSummary: コンテンツサマリー（topicsのみ、自動生成）
  - lastSearchDate: 最後の検索日時
  - searchCount: 検索回数

注意事項:
- topicRelationsテーブルはrelationsテーブルにリネームされました
- topicEmbeddingsテーブルはtopicsテーブルに統合されました
- entityEmbeddings、relationEmbeddingsテーブルは削除され、ChromaDBに統一されました
- companiesテーブルとorganizationCompanyDisplayテーブルは削除され、organizationsテーブルに統合されました
- 埋め込みベクトルはChromaDBにのみ保存されます（SQLiteにはメタデータのみ）
- organizationIdとcompanyIdは排他的（CHECK制約: (organizationId IS NOT NULL AND companyId IS NULL) OR (organizationId IS NULL AND companyId IS NOT NULL)）

各テーブルはID、作成日時、更新日時などの共通フィールドを持ちます。
エンティティとリレーションは組織IDまたは会社IDで紐づけられ、組織/会社ごとにデータを分離しています。
    `.trim(),
    'chromadb-schema': `
