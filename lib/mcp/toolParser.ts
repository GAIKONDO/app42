    ...
    try {
      const request: MCPToolRequest = {
        ...
        arguments: {
          ...
          // format_meeting_note_content Toolの場合、rawContentが指定されていない場合はユーザーの入力テキストを使用
          ...(toolCall.tool === 'format_meeting_note_content' && !toolCall.arguments.rawContent && context?.rawContent && {
      ...
      // デバッグ: format_meeting_note_content Toolの場合、詳細をログ出力
      if (toolCall.tool === 'format_meeting_note_content') {
        console.log('[executeToolCalls] format_meeting_note_content Tool呼び出し:', {
          hasRawContentInArgs: !!toolCall.arguments.rawContent,
          hasRawContentInContext: !!context?.rawContent,
          rawContentLength: (request.arguments.rawContent as string)?.length || 0,
          meetingNoteId: request.arguments.meetingNoteId,
          itemId: request.arguments.itemId,
        });
      }
      ...
      console.log(`[executeToolCalls] Tool呼び出し:`, {
        ...
        hasOrganizationId: !!request.arguments.organizationId,
        hasParentId: !!request.arguments.parentId,
      });
      
      // create_organization Toolの場合、詳細をログ出力
      if (toolCall.tool === 'create_organization') {
        console.log('[executeToolCalls] create_organization Tool呼び出し:', {
          name: request.arguments.name,
          parentId: request.arguments.parentId,
          level: request.arguments.level,
          levelName: request.arguments.levelName,
        });
      }
...
export function formatToolCallResults(
  ...
  const formattedResults = results.map(({ toolCall, result }) => {
    if (result.success && result.data) {
      ...
      // format_meeting_note_content Toolの場合、特別なフォーマット
      if (toolCall.tool === 'format_meeting_note_content') {
        const formattedContent = (data as any)?.formattedContent || '';
        const saved = (data as any)?.saved || false;
        const message = (data as any)?.message || '';
        
        let formatted = `## 議事録整形結果\n\n`;
        ...
        if (saved) {
          formatted += `✅ 議事録の内容を更新しました。`;
        }
        
        return formatted;
      }
      
      // create_organization Toolの場合、特別なフォーマット
      if (toolCall.tool === 'create_organization') {
        const message = (data as any)?.message || '';
        const orgName = (data as any)?.name || '';
        const orgId = (data as any)?.id || '';
        const parentId = (data as any)?.parentId || null;
        
        let formatted = `## 組織作成結果\n\n`;
        ...
        if (orgName) {
          formatted += `- 組織名: ${orgName}\n`;
        }
        if (orgId) {
          formatted += `- 組織ID: ${orgId}\n`;
        }
        if (parentId) {
          formatted += `- 親組織ID: ${parentId}\n`;
        }
      ...
      // get_organization_tree Toolの場合、特別なフォーマット
      if (toolCall.tool === 'get_organization_tree') {
        const message = (data as any)?.message || '';
        const tree = (data as any)?.tree;
        
        let formatted = `## 組織ツリー取得結果\n\n`;
        ...
        if (tree) {
          const orgName = tree.name || '';
          const orgId = tree.id || '';
          const childrenCount = tree.children ? tree.children.length : 0;
          
          formatted += `- 組織名: ${orgName}\n`;
          formatted += `- 組織ID: ${orgId}\n`;
          formatted += `- 子組織数: ${childrenCount}\n`;
        }
